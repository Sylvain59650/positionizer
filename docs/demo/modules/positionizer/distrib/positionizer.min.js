...
var Positionizer = function Positionizer() {};

var __internal = {
  getAllStyles: function getAllStyles(element) {
    return window.getComputedStyle(element);
  },

  getStyle: function getStyle(element, prop) {
    return __internal.getAllStyles(element)[prop];
  },

  isStaticPositioned: function isStaticPositioned(element) {
...
Positionizer.getPosition = function (element) {
  ...
  if (__internal.getStyle(element, "position") === "fixed") {
    ...
    var offsetParentEl = __internal.offsetParent(element);
...
Positionizer.getAttach = function (el, placement) {
  ...
  var w = parseFloat(el.offsetWidth);
  var h = parseFloat(el.offsetHeight);
  switch (placement) {
    case "left-top":
    case "top-left":
      return { left: x, top: y };
...
Positionizer.getRelativePosition = function (fixedEl, floatingEl, placement, appendToBody, distX, distY) {
  var fixedElPos = appendToBody ? __internal.getOffset(fixedEl, false) : Positionizer.getPosition(fixedEl, false);
  var floatingElStyles = __internal.getAllStyles(floatingEl);
  var bcr = floatingEl.getBoundingClientRect();
  var sp = placement.split("-");
  var placementPrimary = sp[0] || "top";
  var outside = sp.length === 3 ? "-outside" : "";
  var placementSecondary = (sp[1] || "") + outside || "center";
  distX = distX || 10;
  distY = distY || 10;
  var floatingElPos = {
  ...
  switch (placementPrimary) {
    case "top":
      floatingElPos.top = fixedElPos.top - (floatingEl.offsetHeight + parseFloat(floatingElStyles.marginBottom)) - distY;
      break;
    case "bottom":
      floatingElPos.top = fixedElPos.top + fixedElPos.height + distY;
      break;
    case "left":
      floatingElPos.left = fixedElPos.left - (floatingEl.offsetWidth + parseFloat(floatingElStyles.marginRight)) - distX;
      break;
    case "right":
